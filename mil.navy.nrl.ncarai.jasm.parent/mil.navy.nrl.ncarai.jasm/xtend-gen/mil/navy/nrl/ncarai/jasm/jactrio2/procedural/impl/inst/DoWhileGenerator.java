...
public class DoWhileGenerator implements IProductionGenerator {
  @Override
  public void generate(final Pattern currentGoal, final Pattern nextGoal, final Instruction inst, final ProceduralGenerationContext context) {
    if ((inst instanceof DoWhileStatement)) {
      ...
      instructions.addAll(((DoWhileStatement)inst).getBlock().getInstructions());
      instructions.add(((DoWhileStatement)inst).getWhile());
      context._goalStateProvider.startBranch(instructions, context);
      Pattern interimGoal = context._goalStateProvider.nextGoalState();
      context.getConsumer().accept(this.createInitialRepeatProduction(currentGoal, interimGoal, ((DoWhileStatement)inst), context));
      context._productionSequencer.sequence(((DoWhileStatement)inst).getBlock().getInstructions(), context);
      context._goalStateProvider.nextState();
      interimGoal = context._goalStateProvider.currentGoalState();
      context.getConsumer().accept(
        this.createTerminalRepeatProduction(interimGoal, currentGoal, ((DoWhileStatement)inst), context));
      context.getConsumer().accept(
        this.createFallthroughRepeatProduction(interimGoal, nextGoal, ((DoWhileStatement)inst), context));
      context._goalStateProvider.endBranch();
    }
  }
  
  public Production createFallthroughRepeatProduction(final Pattern currentGoal, final Pattern targetGoal, final DoWhileStatement until, final ProceduralGenerationContext context) {
  ...
  public Production createTerminalRepeatProduction(final Pattern currentGoal, final Pattern nextGoal, final DoWhileStatement until, final ProceduralGenerationContext context) {
    Production _xblockexpression = null;
    {
      final Production rtn = ModelFragmentFactory.eINSTANCE.createProduction();
      String _computeBaseName = this.computeBaseName(context);
      ...
      final Map<Buffer, BufferPattern> conditions = ProcGenUtilities.mergePatterns(currentGoal, until.getWhile().getBindings().getBindings(), context);
  ...
  public Production createInitialRepeatProduction(final Pattern currentGoal, final Pattern nextGoal, final DoWhileStatement until, final ProceduralGenerationContext context) {
    Production _xblockexpression = null;
    {
      final Production rtn = ModelFragmentFactory.eINSTANCE.createProduction();
      String _computeBaseName = this.computeBaseName(context);
  ...
  public String computeBaseName(final ProceduralGenerationContext context) {
    String _xblockexpression = null;
    {
      ...
      _xblockexpression = (functionName + "-dowhile");
    }
    return _xblockexpression;
  }
  ...
  public boolean handles(final Instruction instr) {
    return (instr instanceof DoWhileStatement);
  }
}
